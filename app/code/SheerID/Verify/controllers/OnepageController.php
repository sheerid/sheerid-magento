    ...
    public function saveVerifyAction() {
		...
		if ($verify_result && $quote->getSheeridRequestId() && $quote->getSheeridResult()) {
			...
			if ($verify_result["awaiting_upload"]) {
				...
				$errors =  $this->__("Unable to verify. Please check that your information is correct.");
			}
			$result['message'] = $verify_result && $verify_result['message'] ? $verify_result['message'] : $errors;
		}
